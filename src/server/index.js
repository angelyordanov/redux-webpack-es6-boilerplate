...
import { createServerRenderer } from 'client/js/app/serverRenderer';

const htmlTemplate =
  fs.readFileSync(fspath.join(__dirname, 'index.html'), 'utf-8');

const serverRenderer = createServerRenderer(htmlTemplate);

const serverRenderMiddleware = (req, res, next) => {
  serverRenderer(req.url).then(({ html, redirect }) => {
    if (redirect) {
